	...
	protected void addAdditionalRecipes(Level level, List<MachineRecipe> list) {
		for (String s : FTBICConfig.AUTO_METALS) {
			...
			if (ingotTag != null && plateTag != null && FTBICConfig.ADD_PLATE_RECIPES) {
				...
				if (id != null) {
					MachineRecipe recipe = new MachineRecipe(recipeSerializer.get(), new ResourceLocation(FTBIC.MOD_ID, "compressing/generated/plate_from_ingot/" + id.getNamespace() + "/" + id.getPath()));
		...
		for (String s : FTBICConfig.AUTO_METALS) {
			...
			if (gearTag != null && plateTag != null && FTBICConfig.ADD_GEAR_RECIPES) {
				...
				if (id != null) {
					MachineRecipe recipe = new MachineRecipe(recipeSerializer.get(), new ResourceLocation(FTBIC.MOD_ID, "compressing/generated/gear_from_ingots/" + id.getNamespace() + "/" + id.getPath()));
					recipe.inputItems.add(new IngredientWithCount(Ingredient.of(plateTag), 3));
					recipe.outputItems.add(new StackWithChance(new ItemStack(item), 1D));
					list.add(recipe);
				}
			}
		...
		for (String s : FTBICConfig.AUTO_GEMS) {
			...
			if (gearTag != null && gemTag != null && FTBICConfig.ADD_GEAR_RECIPES) {
				Item item = FTBICConfig.getItemFromTag(gearTag);
				ResourceLocation id = item == Items.AIR ? null : item.getRegistryName();

				if (id != null) {
					MachineRecipe recipe = new MachineRecipe(recipeSerializer.get(), new ResourceLocation(FTBIC.MOD_ID, "compressing/generated/gear_from_gems/" + id.getNamespace() + "/" + id.getPath()));
					recipe.inputItems.add(new IngredientWithCount(Ingredient.of(gemTag), 3));
					recipe.outputItems.add(new StackWithChance(new ItemStack(item), 1D));
					list.add(recipe);
				}
			}
